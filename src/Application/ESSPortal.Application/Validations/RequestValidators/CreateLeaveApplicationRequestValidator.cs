...
namespace ESSPortal.Application.Validations.RequestValidators.Leave;
...
public class CreateLeaveApplicationRequestValidator : AbstractValidator<CreateLeaveApplicationRequest>
{
    ...
    public CreateLeaveApplicationRequestValidator(IServiceManager serviceManager, ICacheService cacheService) 
    {
        ...
        RuleFor(x => x)
            .MustAsync(MeetGenderRequirementsAsync) // Use the new async method
    ...
    private bool MeetGenderRequirements(CreateLeaveApplicationRequest request)
    {
        ...
        var userInfo = _cacheService.GetUserInfo(request.EmployeeNo);
        if (userInfo == null) throw new Exception("Missing gender information.");
    ...
    private async Task<bool> MeetGenderRequirementsAsync(CreateLeaveApplicationRequest request, CancellationToken cancellationToken)
    {
        ...
        // ... (Include the rest of the original logic for MaxDays checks if needed) ...
    ...
    private async Task<LeaveSummaryResponse?> GetLeaveSummary(string employeeNo)
    {
        ...
        if (cachedDashboardData?.LeaveSummary is not null)
        {
