...
internal sealed class NavisionService : INavisionService
{
    ...
    public async Task<AppResponse<(T, string RawJson)>> CreateAsync<T>(string serviceName, T entity) where T : class, new()
    {
        try
        {
            ...
            return result != null
                ? AppResponse<(T, string RawJson)>.Success("Success", (result, responseJson))
                : AppResponse<(T, string RawJson)>.Failure("Failed to create entity.");
        }
        catch (Exception)
        {
    ...
    public async Task<AppResponse<(T, string RawJson)>> CreateLeaveApplicationAsync<T>(string serviceName, T entity) where T : class, new()
    {
        try
        {
            ...
            if(!response.IsSuccessStatusCode) 
            {
                ...
                return AppResponse<(T, string RawJson)>.Failure($"Failed to create entity: {response} {errorJson}");
            }
    ...
    public async Task<AppResponse<(T, string RawJson)>> UpdateLeaveApplicationAsync<T>(string serviceName, T entity) where T : class, new()
    {
        try
        {
            var url = GetServiceUrl(serviceName);
    ...
    public async Task<AppResponse<List<T>>> CreateMultipleAsync<T>(string serviceName, List<T> entities) where T : class, new()
    {
        try
        {
            ...
            {
                return AppResponse<List<T>>.Failure("Failed to create entities or no entities returned.");
            }

            //ToDo: Assuming the API returns the created entities, otherwise you can return a success message without the entities

            return result != null
                ? AppResponse<List<T>>.Success("Success", result)
                : AppResponse<List<T>>.Failure("Failed to create entities.");
    ...
    public async Task<AppResponse<T>> GetSingleAsync_<T>(string requestUri) where T : class, new()
    {
        try
        {
            ...
            if (result == null)
            {
                return AppResponse<T>.Failure("Entity not found or failed to deserialize.");
            }
    ...
    public async Task<AppResponse<T>> GetSingleAsync<T>(string requestUri) where T : class, new()
    {
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                return AppResponse<T>.Failure($"Business Central API returned {response.StatusCode}: {errorContent}");
            }
    ...
    public async Task<AppResponse<(List<T>, string RawJson)>> GetMultipleAsync_<T>(string requestUri) where T : class, new()
    {
        try
        {
    ...
    public async Task<AppResponse<(List<T>, string RawJson)>> GetMultipleAsync<T>(string requestUri) where T : class, new()
    {
        try
        {
            ...
            {
                var errorContent = await response.Content.ReadAsStringAsync();
                return AppResponse<(List<T>, string)>.Failure($"Business Central API returned {response.StatusCode}: {errorContent}");
            }
    ...
    public async Task<AppResponse<(List<T>, string RawJson)>> GetMultipleAsyncSimple_<T>(string requestUri) where T : class, new()
    {
        try
        {
            var response = await _httpClient.GetAsync(requestUri);
    ...
    public async Task<AppResponse<(List<T>, string RawJson)>> GetMultipleAsyncSimple<T>(string requestUri) where T : class, new()
    {
        try
        {
    ...
    public async Task<AppResponse<bool>> PostAsync(string requestUri)
    {
        try
        {
            ...
            return AppResponse<bool>.Success("Operation successful", true);
        }
        catch (Exception)
        {
    ...
    public async Task<AppResponse<bool>> PostAsync(string requestUri, Dictionary<string, string> parameters)
    {
        try
        {
            var response = await _httpClient.PostAsync(requestUri, null);
    ...
    public async Task<AppResponse<bool>> PostAsync<T>(string serviceName, T filter) where T : class, new()
    {
        try
        {
