...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('submit', function (event) {
        if (event.target.id === 'payslipGenerationForm') {
            ...
            const cancelBtn = modal ? modal.querySelector('#payslipCancelBtn') : null;
            ...
            // Submit via AJAX (returns JSON response)
            fetch(form.action, {
                method: 'POST',
                body: formData,
                headers: {
                ...
                .then(result => {
                    ...
                    if (result.success) {
                        console.log('Success! Starting download...');
    ...
    // Helper function to disable modal controls (ONLY during form submission)
    ...
    // Helper function to enable modal controls (restore normal behavior)
    ...
    function disableModalControls_(submitBtn, closeBtn, cancelBtn, modal) {
        // Disable submit button
        if (submitBtn) {
            submitBtn.disabled = true;
            submitBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>Generating...';
        ...
        // Disable modal backdrop and ESC
        if (modal) {
            const modalInstance = bootstrap.Modal.getInstance(modal);
            if (modalInstance) {
                modal.setAttribute('data-original-backdrop', modalInstance._config.backdrop);
                modal.setAttribute('data-original-keyboard', modalInstance._config.keyboard);

                modalInstance._config.backdrop = 'static';
                modalInstance._config.keyboard = false;
            }

            // Block modal hide attempts
            const blockModalHide = function (e) {
                e.preventDefault();
                e.stopPropagation();
                e.stopImmediatePropagation();
            ...
            modal.addEventListener('hide.bs.modal', blockModalHide);
            modal.setAttribute('data-hide-blocker', 'active');
        }
    }

    // Helper function to enable modal controls
    function enableModalControls_(submitBtn, closeBtn, cancelBtn, modal) {
        console.log('Resetting modal controls...');

        // Remove modal hide blocker first
        if (modal && modal.getAttribute('data-hide-blocker') === 'active') {
            const blockModalHide = function (e) {
                e.preventDefault();
                e.stopPropagation();
                e.stopImmediatePropagation();
            ...
            modal.removeEventListener('hide.bs.modal', blockModalHide);
            modal.removeAttribute('data-hide-blocker');
        }
        ...
        // Re-enable modal backdrop and keyboard
        if (modal) {
            const modalInstance = bootstrap.Modal.getInstance(modal);
            if (modalInstance) {
                const originalBackdrop = modal.getAttribute('data-original-backdrop');
    ...
    // Helper function to download base64 file (for modal form)
