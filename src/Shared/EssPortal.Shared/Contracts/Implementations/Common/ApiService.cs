...
using EssPortal.Shared.Utilities;
...
namespace ESSPortal.Shared.Contracts.Implementations.Common;
...
public class ApiService : IApiService
{
    ...
    public async Task<AppResponse<TResponse>> HandlePostRequest<TRequest, TResponse>(string endpoint, TRequest request)
    {
        if (string.IsNullOrWhiteSpace(endpoint))
            return AppResponse<TResponse>.Failure("Endpoint not configured.");

        endpoint = EndpointHelper.ReplaceVersion(endpoint, _apiSettings.Version);
        var apiResponse = await PostAsync<TRequest, TResponse>(endpoint, request);
    ...
    public async Task<AppResponse<TResponse>> HandlePutRequest<TRequest, TResponse>(string endpoint, TRequest request)
    {
        if (string.IsNullOrWhiteSpace(endpoint))
            return AppResponse<TResponse>.Failure("Endpoint not configured.");

        endpoint = EndpointHelper.ReplaceVersion(endpoint, _apiSettings.Version);
        var apiResponse = await PutAsync<TRequest, TResponse>(endpoint, request);
    ...
    public async Task<AppResponse<T>> HandleDeleteRequest<T>(string endpoint)
    {
        if (string.IsNullOrWhiteSpace(endpoint))
            return AppResponse<T>.Failure("Endpoint not configured.");

        endpoint = EndpointHelper.ReplaceVersion(endpoint, _apiSettings.Version);
        var apiResponse = await DeleteAsync<T>(endpoint);
    ...
    public AppResponse<T> ConvertApiResponse<T>(AppResponse<T?> apiResponse)
    {
    ...
    private static JsonSerializerOptions CreateResilientJsonOptions()
    {
        ...
        options.Converters.Add(new ResilientEnumConverterFactory());
